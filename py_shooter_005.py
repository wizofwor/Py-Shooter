...
class Enemy(Actor):
	...
	def update(self):
		...
		print(math.sin(math.radians(self.rect.y)))
		# Uçak ekranın sonuna geldiğini uçağı öldür.
...
class main():
	...
	def __init__(self):
		...
		self.player = Player(self.sheet.get_image("playerShip3_blue.png"))
		...
		# Kullanıcak font
		self.font = pg.font.SysFont('Consolar.ttf', 24)
	...
	def game_loop(self):
		...
		while True:
			...
			if (self.game_time%500==0):
				enemy = Enemy(self.sheet.get_image("enemyBlack2.png"))
			...
			# Zaman
			self.game_time += 1
			game_time_text = str(self.game_time//100).rjust(3)
			...
			# Zamanı güncelle
			self.screen.blit(self.font.render(game_time_text, True, (0, 128, 0)), (180, 10))
			pg.display.flip()
